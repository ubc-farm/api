import {
	extend, selectiveExtend, convert,
...
import {
	...
	getHiddenDurationBefore, getHiddenDurationBetween
} from './DateUtil.js';
...
export default class Range extends Component {
		...
		if (animation) { // true or an Object
			...
			if (!easingFunction) {
				throw new Error('Unknown easing function ' + JSON.stringify(easingName) + '. ' +
						'Choose from: ' + Object.keys(easingFunctions).join(', '));
			}
	...
	_applyRange(start, end) {
		var newStart = (start != null) 
			?	convert(start, 'Date').valueOf() : this.start;
		var newEnd = (end != null) 
			? convert(end, 'Date').valueOf() : this.end;
		var max = (this.options.max != null) 
			?	convert(this.options.max, 'Date').valueOf() : null;
		var min = (this.options.min != null) 
			? convert(this.options.min, 'Date').valueOf() : null
		var diff;
	...
	_onDrag(event) {
		...
		const diffRange = this.options.rtl 
	...
	_isInsideRange(event) {
		...
		const x = this.options.rtl 
			? clientX - getAbsoluteLeft(this.body.dom.centerContainer)
			: getAbsoluteRight(this.body.dom.centerContainer) - clientX;
