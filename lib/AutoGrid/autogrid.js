...
export default function* AutoGrid(container, baseDimensions, angle = 0.0) {
	if (!container.getGeometryType) 
		throw Error('AutoGrid container must be a JSTS geometry');
	const width = new DefaultMap(baseDimensions.width || 1.0);
	const height = new DefaultMap(baseDimensions.height || 1.0);
