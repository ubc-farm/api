...
const HeaderCell = props => {
	...
	return (
		<Cell header className='th-hoverable'align={align} onClick={onClick}>
...
export default class Head {
	...
	constructor(props) {
		...
		this.onClicks = new Map();
		if (onColumnClick) {
			for (const {columnKey} of props.columns) 
				this.onClicks.set(columnKey, () => onColumnClick(columnKey));
		}
	}
				...
				<tr className='table-th-row'>
					...
					{columns.map(({columnKey, description, title, align, useSorting}) => (
						<HeaderCell key={columnKey}
							...
							onClick={this.onClicks.get(columnKey)}
