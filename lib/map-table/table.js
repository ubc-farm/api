...
export class Table extends Array {
	 ...
	 * If the value can't be found, false is returned.
	 * @param {*} value to search for
	 * @param {*} column - only search a certain column
	 * @returns {Array|boolean} where [0] is the row index, 
	 * and [1] is the column key. false if the value doesn't exist
	 */
	...
	/** @returns {boolean} alias for indexOf that only returns booleans */
	has(value, column) {return this.indexOf(value, column) !== false;}
	...
	/**
	 * JSON transformer that turns each map into a JSON object.
	 * Calls replacer property of the Table to transform the map entries.
	 * @returns {Array<Object>} serializable object
	 */
	toJSON() {
		return this.map(row => {
	...
	/**
	 * Transforms JSON into a table. 
	 * @param {string|Array} data - json to parse
	 */
	static parse(data) {
