  ...
  class InteractiveTable extends React.Component {
  	constructor(props) {
  		...
  		if (sorting) {
  			...
  			if (typeof sorting === 'object' && sorting.columnKey) state.sort.columnKey = sorting.columnKey;else {
  				const { data } = props;let firstColumnKey;
  				for (const key in data[0]) {
  					firstColumnKey = key;break;
  				}
  	...
  	render() {
  		...
  		if (settings.selected) {
  			columns.push(buildCheckboxesColumn(data.length, selected));
  		}
  	...
  	columnClickHelper(columnKey) {
  		...
  		if (settings.selected && columnKey === CHECKBOX) {
  			...
  			if (sort.columnKey === columnKey) newState.descending = !sort.descending;else newState.columnKey = columnKey;newState.descending = true;
  	...
  	for (const columnKey in schema) {
  		...
  		yield new Column({
  			...
  			header: ({ align, onClick, columnKey }) => React__default.createElement(
  				...
  				React__default.createElement(
  					'span',
  					{ key: 'hover', title: tooltips[columnKey],
  						className: classList('table-cell-hover', `table-cell-hover-align-${ align }`, { 'visible': thisSorting })
  					},
