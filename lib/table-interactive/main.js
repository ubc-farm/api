...
export default class InteractiveTable extends Component {
	...
	render() {
		const {data, cell, tooltips, extraColumns = []} = this.props;
		...
		columns.push(...buildColumns(
			cell, data, 
			settings.sort ? sort : undefined,
			tooltips
		));
		columns.push(...extraColumns.map(c => {
 ...
 */
function* buildColumns(cell = Cell, data, sortInfo, tooltips = new Map()) {
	...
	for (const columnKey in schema) {
		...
		const thisTooltip = tooltips.get(columnKey);
		let title;
		if (thisTooltip && 'title' in thisTooltip) 
			title = thisTooltip.title;
		else title = columnKey;
		...
		yield new Column({
			...
			header: ({align, onClick, columnKey}) => (
				<Cell header align={align} onClick={onClick} className='th-hoverable'>
					...
					<span key='hover' hidden
						title={thisTooltip && thisTooltip.definition} 
						className={cx(
							'table-cell-hover', 
							`align-${align}`,
							{'visible': thisSorting}
