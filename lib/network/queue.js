...
export default class Queue {
	...
	add(...items) {
		...
		.then(keys => {
			keys.reduce((keySet, value) => keySet.add(value), this.keys);
		})
	}
	 ...
	 */
	clear() {
		return this.dbRequest.then(db => db
	...
	size() {
		return this.dbRequest.then(db => db
			.transaction(this.STORE_NAME).objectStore(this.STORE_NAME)
			.count())
	}
