 ...
 */
const reader, {parser: writer} = new jsts.io.GeoJSONReader(factory);
...
register(msg => {if (msg.gridSpec) {
	const polygon = reader.read(msg.path);
	const gridSpec, {angle} = msg.gridSpec;
		
	return [...AutoGrid(polygon, gridSpec, angle)].reduce((array, cell) => {
